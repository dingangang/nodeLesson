  ...
  socket.on('nameAttempt', function (name) {
    if (name.indexOf('Guest') === 0) {
      ...
      if (namesUsed.indexOf(name) === -1) {
        const perviousName = nickNames[socket.id];
        const perviousNameIndex = namesUsed.indexOf(perviousName);
        namesUsed.push(name);
        nickNames[socket.id] = name;
        delete namesUsed[perviousNameIndex]
        socket.emit('nameResult', {
          success: true,
          name: name
        })
